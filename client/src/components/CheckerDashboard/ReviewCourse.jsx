...
const ReviewCourse = () => {
  ...
  const [resourceModal, setResourceModal] = useState({ show: false, resource: null });
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      try {
        ...
        setAlert({ show: true, variant: "danger", message: "Failed to fetch course." });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      setAlert({ show: true, variant: "success", message: "Course approved and published!" });
      setTimeout(() => navigate("/checker-dashboard"), 1200);
    } catch (err) {
      setAlert({ show: true, variant: "danger", message: "Failed to approve course." });
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      setAlert({ show: true, variant: "danger", message: "Failed to reject course." });
    } finally {
      setActionLoading(false);
    }
  };
...
const renderResources = (resources) => {
  if (!resources || !resources.length) return <span className="text-muted">No resources</span>;
  return (
    <ul>
      {resources.map((res, idx) => {
        // PDF base64
        if (res.type === "document" && res.url && res.url.startsWith("data:application/pdf")) {
  return (
    <li key={idx} className="mb-1">
      ...
      >
        ...
        if (res.type === "document" && res.url && res.url.startsWith("data:")) {
          return (
            <li key={idx} className="mb-1">
              <strong>{res.title || "Untitled"}</strong> (Document)
              <a
                href={res.url}
                download={res.fileDetails?.originalName || res.title || "document"}
        ...
        if (
          (res.type === "video_url" && (res.url.includes("youtube.com") || res.url.includes("youtu.be"))) ||
          res.type === "video_file"
        ) {
          return (
            <li key={idx} className="mb-1">
              ...
              <Button
                ...
                size="sm"
                className="ms-2"
                onClick={() => setResourceModal({ show: true, resource: res })}
              >
        ...
        <>
          ...
          <Table bordered>
            <tbody>
              ...
              <tr>
                <th>Instructor</th>
                <td>{course.instructor?.name || course.instructor?.email || displayValue(course.instructor)}</td>
              </tr>
              <tr>
                <th>Duration</th>
                <td>{displayValue(course.duration)}</td>
              </tr>
              <tr>
                <th>Tags</th>
              ...
              <tr>
                <th>Created At</th>
                <td>{course.createdAt ? new Date(course.createdAt).toLocaleString() : <span className="text-muted">N/A</span>}</td>
              </tr>
              <tr>
                <th>Status</th>
                <td>
              ...
              <tr>
                <th>Published</th>
                <td>
                  {course.published ? <Badge bg="success">Yes</Badge> : <Badge bg="secondary">No</Badge>}
                </td>
              </tr>
            </tbody>
          </Table>
          ...
          <div className="d-flex flex-wrap gap-4 mb-4">
            <div>
              <h6>Thumbnail</h6>
              {course.thumbnail ? (
                <img src={course.thumbnail} alt="thumbnail" style={{ maxWidth: 180, borderRadius: 8, border: "1px solid #ccc" }} />
              ) : (
            ...
            <div>
              ...
              {course.certificate && course.certificate.templateUrl ? (
                <img src={course.certificate.templateUrl} alt="certificate" style={{ maxWidth: 220, borderRadius: 8, border: "1px solid #ccc" }} />
              ) : (
                <span className="text-muted">N/A</span>
              )}
            </div>
              ...
              {course.units.map((unit, unitIdx) => (
                <Accordion.Item eventKey={unitIdx.toString()} key={unitIdx}>
                  ...
                  <Accordion.Body>
                    ...
                    {unit.lessons && unit.lessons.length > 0 ? (
                      <Accordion>
                        {unit.lessons.map((lesson, lessonIdx) => (
                          <Accordion.Item eventKey={lessonIdx.toString()} key={lessonIdx}>
                            <Accordion.Header>
                              Lesson {lessonIdx + 1} Title : {lesson.title || "Untitled"}
                            </Accordion.Header>
                            <Accordion.Body>
                              <div>
                                <strong>Description : </strong>
                                <div>{lesson.content || <span className="text-muted">No content</span>}</div>
                                {lesson.videoUrl && (
                                  <div className="mt-2">
                                    <strong>Video:</strong>
                                    <div>
                                ...
                                <div className="mt-2">
                                  <strong>Duration:</strong> {lesson.duration ? `${lesson.duration} min` : <span className="text-muted">N/A</span>}
                                </div>
                                <div className="mt-2">
                    ...
                    <div className="mt-3">
                      <h6>Assignments</h6>
                      {unit.assignment && unit.assignment.assignmentSets && unit.assignment.assignmentSets.length > 0 ? (
                        <Accordion alwaysOpen>
                          {unit.assignment.assignmentSets.map((set, setIdx) => (
                            <Accordion.Item
                              eventKey={setIdx.toString()}
                              key={setIdx}
                              onClick={() => handleAssignmentSetToggle(unitIdx, setIdx)}
                              active={!!openAssignmentSets[`${unitIdx}-${setIdx}`]}
                            >
                              <Accordion.Header>
                                Set {set.setNumber || setIdx + 1} Title : {set.title || "Untitled"}
                              </Accordion.Header>
                              <Accordion.Body>
                                <div>
                                  <strong>Description:</strong> {set.description || <span className="text-muted">No description</span>}
                                  <br />
                                  <strong>Difficulty:</strong> {set.difficulty || <span className="text-muted">N/A</span>}
                                </div>
                                <div className="mt-3">
                                  <h6>Questions</h6>
                                    ...
                                    <Accordion
                                      activeKey={
                                        openAssignmentSets[`${unitIdx}-${setIdx}`]
                                          ? set.questions.map((_, qIdx) => qIdx.toString())
                                          : []
                                      }
                                      alwaysOpen
                                    >
                                      {set.questions.map((q, qIdx) => (
                                        <Accordion.Item eventKey={qIdx.toString()} key={qIdx}>
                                          <Accordion.Header>
                                            Question {qIdx + 1}
                                          </Accordion.Header>
                                          <Accordion.Body>
                                            <div>
                                              <strong>Question:</strong> {q.questionText || <span className="text-muted">No question</span>}
                                            </div>
                                            <div className="mt-2">
                                              ...
                                              <ul>
                                                {q.options && q.options.length > 0 ? (
                                                  ...
                                                  <li className="text-muted">No options</li>
                                                )}
                                              </ul>
                                            </div>
                                            <div>
                                              <strong>Correct Option:</strong>{" "}
                                              {q.correctAnswer || <span className="text-muted">N/A</span>}
                                            </div>
                                            <div>
